...
if options[:layer]
  ids = layer_volumes(options[:layer], stack(options[:stack]))
  ap ids.each { |id|
    create(options[:region], { :volume_id => id, :description  => options[:description]})
  }
else
  ap create(options[:region], { :volume_id => options[:id], :description  => options[:description]})
end
